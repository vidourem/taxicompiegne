...
function get_unicode($string) {
    ...
    if(is_utf8(substr($string, 0, 512))) return 'UTF-8';
...
*
*/
function vcard_stripcslashes($string, $escapes) {
    $escape_mode = 0;
    $out_string = '';
...
class Contact_Vcard_Parse {
    ...
    */
    ...
    function fromFile($filename, $decode_qp = true, $encoding = 'ISO-8859-1')
    {
        ...
        if ($text === false) {
            ...
            return $this->fromText($text, $decode_qp, $encoding);
        }
    }
    ...
    */
    ...
    function fromText($text, $decode_qp = true, $encoding = 'ISO-8859-1')
    {

        //cwacha: detect input encoding and convert it to utf-8 if necessary
        $enc = get_unicode($text);
        if($enc == false) {
            $enc = $encoding;
            msg("Could not detect encoding of file. Trying with $encoding...", -1);
        } else {
            msg("detected $enc encoding");
        }
        ...
        if($enc != 'UTF-8') {
            if(function_exists('iconv')) {
                ...
                if(strlen($text) == 0) msg("iconv could not convert the text!", -1);
            } else {
                msg("Error on import: the vcard is not in UTF-8 format and iconv is not available. Import canceled.", -1);
                return false;
            }
        }
        // strip BOM if it is still there
        $text = ltrim($text, "\xFE\xFF\xEF\xBB\xBF\0");
        ...
        // Unfortunately Apple used two spaces for folding so we have to
        // check first if we can also remove 2 spaces in all cases. If not
        // it could be that we have a folded line that also contains a space
        // at the beginning - eat one space only in such cases as with compliant
        // vCards.
    ...
    */
    ...
    function unescape(&$text)
    {
        if (is_array($text)) {
            ...
            //cwacha: unescaping MUST NEVER be done with str_replace, because it's context sensitive!!!
            $text = vcard_stripcslashes($text, ":;,n\\");
        ...
        foreach ($source as $line) {
            ...
            if (! $begin) {
                ...
                if (strtoupper($left) == 'END' &&
                    ...
                    case 'CATEGORIES':
                    case 'CATEGORY':
                        //cwacha: Apple AddressBook Hack (CATEGORY)
                    ...
                    if($item_nr == 0) {
                        ...
                        if(empty($item[$item_nr])) {
                            //echo "first occurence of item: $item_nr ($typedef)<br>";
                            // this is the first occurence of this item. Remeber the typedef and the item nr
                            ...
                            //echo "addon at item: $item_nr ($item[$item_nr]) of typedef $typedef<br>";
                            // search the original typedef in the card for the entry with this item number
                            foreach($card[ $item[$item_nr] ] as $key => $elem) {
                                // and append the current data as sub-array to this entry
                                if($elem['item'] == $item_nr) {
                                    $card[$item[$item_nr]][$key][$typedef] = array(
                                        'param' => $params,
                                        'value' => $value
                                    );
                                }
    ...
    */
    ...
    function _getTypeDef($text)
    {
        ...
        // cwacha: deal with itemX.ADR stuff
